...
const ingredient = ({
  ...
  getters: {
    ...
    searchIngredient: (state) => state.searchIngredient,
  ...
  mutations: {
    ...
    searchIngredient(state,ingredient){
      state.searchIngredient = ingredient
    },
  ...
  actions: {
    ...
    getIngredientById({ commit }, objData) {
      commit('setLoading', true)
      commit('setError', null)
      return new Promise((resolve, reject) => {
        axios
          .get(apiEndpoints.ingredients.getIngredientById(objData))
          .then(
            //Якщо добре
            (res) => res.data
          )
          .then((resData) => {
            if (resData.success) commit('searchIngredient', resData.data)
            else throw new Error('Fatch failed!')
          })
          .catch((err) => {
            //Якщо погано

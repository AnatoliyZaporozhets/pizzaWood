...
const store = {
  ...
  mutations: {
    ...
    setAuthData(state, { authData, expiresAt }) {
      ...
      state.expiresAt =
        expiresAt || state.authData.expires_in * 10000 + new Date().getTime()
  ...
  actions: {
    ...
    signup({ commit }, { name, email, password , admin}) {
      return new Promise((resolve, reject) => {
        axios
          .post(apiEndpoints.user.signup, { name, email, password , admin })
          .then(function () {
  ...
  getters: {
    ...
    admin: (state) => () => {
      return state.authData.admin && new Date().getTime() < state.expiresAt
    },

          ...
          <form @submit.prevent="submit">
            ...
            <div>
              ...
              class="loginContainerInput"
                ...
                placeholder="Name"
              />
              <div v-if="checkNameMessage">{{ message }}</div>
            </div>
            ...
            <div class="form-group">
              ...
              />
              <div v-if="checkPasswordMessage">{{ message }}</div>
            </div>
           
            <div>
              <button :disabled='message' class="loginContainerBtn" type="submit">Sign up</button>
            </div>
          </form>
        </div>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      password: "",
      admin: false,
      checkNameMessage: false,
      checkPasswordMessage: false,
      message:''
    };
  },
  watch: {
    userName() {
      if ( this.userName.length === 0 || (this.userName.length > 2 && this.userName.length < 12)   ) {
        this.checkNameMessage = false
        
      }
      else {
        this.checkNameMessage = true
        this.message = 'Ім\'я повинен містити від 3 до 11 символів'
        }
    },
    password() {
      if ( this.userName.length === 0 || (this.userName.length > 4 && this.userName.length < 12)   ) {
        this.checkPasswordMessage = false
      }
      else {

        this.checkPasswordMessage = true
